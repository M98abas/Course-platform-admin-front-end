...
import { Copon } from '../../interfaces'
import { useRouter } from 'next/router'

const TableSampleClients = ({ columns }) => {
const router: any = useRouter()
  const { clients } = useSampleClients('copon')
...
const handleCancelAction = () => {
  ...
  const handelDeleteAction = async () => {
    setLoading(true)
    await ApiDeleteData('copon', id, (data) => {
      if (!data)        
return (
          <>
...
// router.reload()
  ...
  const handleModalAction = async () => {
    ...
    await ApiUpdateData(
      'copon',
      { id, text, percentage, constValue, target, availableFor, active },
      (data) => {
        if (data.error)
          return (
            <>
      ...
      {!clients ? (
	    ...
	    onCancel={() => handleCancelAction}
              ...
              <div className="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                  <label
                    ...
                    className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                  >
                    How much percentage for this copon?
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    defaultValue={clients[ind]?.active}
          ...
          <table className="P-4">
            ...
            <tbody>
              {clientsPaginated.map((client: Copon, index: number) => (
                <tr key={client.id}>
                  <td className="border-b-0 lg:w-6 before:hidden"> {client.text} </td>
                  <td data-label="nickName">{client.percentage}</td>
                  <td data-label="Name">{client.constValue}</td>
                  <td data-label="Email">{client.target}</td>
                  <td data-label="PhoneNumber">{client.availableFor}</td>
