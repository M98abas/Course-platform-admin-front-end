...
const TableSampleClients = ({ columns, subCourseData }) => {
  const { clients } = useSampleClients(`token`)
const router: any = useRouter()
  ...
  const options: any = {
    ...
    defaultDate: new Date(clients[ind]?.expire_at || '2022-01-01'),
    language: 'en',
  }
  ...
  const handelDeleteAction = async () => {
    setLoading(true)
    await ApiDeleteData('token', id, (data) => {
      if (data.errMsg != '')
        return (
          <>
...
//    router.reload()
  ...
  const handleModalAction = async () => {
    setLoading(true)
    try {
      await ApiUpdateData('token', { id, expire_at }, (data) => {
      ...
      {!clients ? (
        ...
        <>
              ...
              <div className="grid gap-6 mb-6 md:grid-cols-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    defaultValue={clients[ind]?.subCoursesId}
                    id="countries"
                    disabled
          ...
          <table>
            ...
            <tbody>
              {clientsPaginated.map((client: Token, index: number) => (
                <tr key={client.id}>
                  <td data-label="nickName">{client.token}</td>
                  <td data-label="Name">{client.expire_at}</td>
                  <td data-label="PhoneNumber">{client.course.titleAr}</td>
