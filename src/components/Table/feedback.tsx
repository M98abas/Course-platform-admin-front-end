...
const TableSampleClients = ({ columns }) => {
  const { clients } = useSampleClients(`feedback`)
const router: any = useRouter()
 ...
 const handelDeleteAction = async () => {
    setLoading(true)
    await ApiDeleteData('feedback', ids, (data) => {
...
//    router.reload()
  ...
  const handleModalAction = async () => {
    setLoading(true)
    await ApiUpdateData('feedback', { id: ids, title, target }, (data) => {
      if (data.error != '')
        return (
          <>
            <div className="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
          ...
          <table>
            ...
            <tbody>
              {clientsPaginated.map((client: Client, index: number) => (
                <tr key={client.id}>
<td>{!client.isAudio?
<img alt={client.title} src={client.url} width={64} height={70} /> :''
}
</td> 
                 <td data-label="nickName">{client.title}</td>
                  <td data-label="Name">{client.target}</td>
                  <td data-label="Email">{client.Course?.titleEn}</td>
