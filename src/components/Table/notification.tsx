...
import { mdiAccountReactivate, mdiEye, mdiTrashCan,mdiCheckCircle ,mdiClockAlertOutline } from '@mdi/js'
...
const TableSampleClients = ({ columns }) => {
  ...
  const options: any = {
    ...
    defaultDate: new Date(clients[ind]?.sendDate || '2022-01-01'),
    language: 'en',
  }
...
//    router.reload()
  ...
  const handleModalAction = async () => {
    setLoading(true)
    try {
      await ApiUpdateData('notification', { id, sendDate, message, title, target }, (data) => {
                ...
                <div>
                  ...
                  <div>
                    <div className="relative max-w-sm">
                      <Datepicker
                        options={options}
                        onChange={(dateSelected: any) => setSendDate(dateSelected)}
                        show={show}
                  ...
                  <div>
                    <div className="relative max-w-sm">
                      <input
                        ...
                        defaultValue={clients[ind]?.message}
                        id="levelOfExperience"
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Any data"
                        required
            ...
            <tbody>
              {clientsPaginated.map((client: Notification, index: number) => (
                <tr key={client.id}>
                  ...
                  <td data-label="PhoneNumber"><Icon path={client.status=='Pending' ? mdiClockAlertOutline : mdiCheckCircle} size={64} /></td>
