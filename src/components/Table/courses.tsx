...
import {ApiActivateData, ApiDeleteData, ApiUpdateData } from '../../api/api'
...
const TableSampleClients = ({ columns }) => {
  const { clients } = useSampleClients('course')
 ...
 const [isModalActivateActive, setIsModalActivateActive] = useState(false)
  ...
  const handelDeleteAction = async () => {
    setLoading(true)
    await ApiDeleteData('course', id, (data) => {
      console.log(data)
...
const handelActivationAction = async () => {
    setLoading(true)
    await ApiActivateData('course', id, (data: any) => {
      if (data)
        return (
          <>
            <div className="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
...
router.reload()  
  ...
  const handelRouteClick = async(cid: any,title:any) => {
	await Cookies.set('course',title);    
router.push({
      pathname: `/subcourse/${cid}`,
    })
  }
      ...
      {!clients ? (
        ...
        <>
          ...
          <CardBoxModal
            ...
            onConfirm={handelActivationAction}
            onCancel={() => handleCancelAction()}
          >
            <p>
              Are you sure you want to Activate this <b> {clients[ind]?.titleAr} </b> ??
            </p>
          </CardBoxModal>
  <CardBoxModal
            title="Please confirm"
          ...
          <table>
            ...
            <tbody>
              {clientsPaginated.map((client: Client, index: number) => (
                <tr key={client.id}>
                  <td className="transition duration-300 ease-in-out delay-150 border-b-0 rounded-md hover:text-white-900 hover:-translate-y-1 hover:scale-110 hover:bg-red-400 lg:w-6 before:hidden">
                    <p className="cursor-pointer" onClick={() => handelRouteClick(client.id,client.titleEn)}>
                      View
                    </p>
                  </td>
                  <td className="border-b-0 lg:w-6 before:hidden">
                    {client.imgUrl && (
                      <img
                        src={client.imgUrl}
                        className="transition hover:scale-400"
                        width={120}
                        height={120}
                        alt="Images"
                      />
