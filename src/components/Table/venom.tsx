...
const TableSampleClients = ({ columns }) => {
  const { clients } = useSampleClients(`venomData`)
const router: any = useRouter()

  const [ids, setids] = useState()
  const [traders, setTraders] = useState(0)
  const [users, setUsers] = useState(0)
  const [toper, setToper] = useState(0)
  ...
  const handelDeleteAction = async () => {
    setLoading(true)
    await ApiDeleteData('venomData', ids, (data) => {
...
//    router.reload()
  ...
  const handleModalAction = async () => {
    setLoading(true)
    await ApiUpdateData('venomData', { id: ids, traders, users, toper }, (data) => {
console.log(data)      
if (data.error)
        return (
          <>
              ...
              <div className="grid gap-6 mb-6 md:grid-cols-3">
                <div>
                  ...
                  <input
                    type="number"
                    id="name"
                    defaultValue={clients[ind]?.traders}
                ...
                <div>
                  ...
                  <input
                    ...
                    id="phoneNumber"
                    defaultValue={clients[ind]?.users}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    onChange={(e) => setToper(parseInt(e.target.value))}
                    id="phoneNumber"
                    defaultValue={clients[ind]?.toper}
                    className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="Any data"
                    required
                  />
          ...
          <table>
            ...
            <tbody>
              {clientsPaginated.map((client: Client, index: number) => (
                <tr key={client.id}>
                  <td data-label="nickName">{client.traders}</td>
                  <td data-label="Name">{client.users}</td>
                  <td data-label="Email">{client.toper}</td>
