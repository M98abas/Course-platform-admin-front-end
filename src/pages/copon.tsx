...
import TableSampleClients from '../components/Table/copon'
...
const TablesPage = () => {
  ...
  const handleModalAction = async () => {
    setError(false)
    if (text == '' && percentage == 0 && constValue == 0 && target == '' && availableFor == 0) {
      setError(true)
      return
    }
    setLoading(true)
    await ApiAddData('copon', { text, percentage, constValue, target, availableFor }, (data) => {
      if (data.error)
        return (
          <>
  ...
  return (
    <>
      {!Loading ? (
        <>
          <Head>
            <title>{getPageTitle('Copons')}</title>
          </Head>
          <SectionMain>
            <SectionTitleLineWithButton icon={mdiTableBorder} title="Copon" main>
              <Button
                ...
                <form>
                  <div className="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                      ...
                      <select
                        onClick={(e: any) => setTarget(e.target.value)}
                        id="countries"
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                      >
                        <option value="subCourse" selected>
                          Select One
                        </option>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="How much this copon available"
                        required
                      />
                    </div>
                  </div>
