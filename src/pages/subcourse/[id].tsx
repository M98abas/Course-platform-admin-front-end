...
import TableSampleClients from '../../components/Table/subCourse'
import { firebaseConfig, getPageTitle } from '../../config'
...
const SubCourse = () => {
  ...
  const pathIndex = `${Cookies.get('course')} / Sub-course`

  useEffect(() => {
    // Add the current route to the history whenever the route changes
    setRouteHistory((prevRouteHistory) => [...prevRouteHistory, router.asPath])
  }, [router.asPath])
  ...
  const handleModalAction = async () => {
    ...
    await ApiAddData(
      'subCourse',
      { titleAr, titleEn, price, descriptionAr, descriptionEn, imgUrl, isWorkshop, id },
      (data) => {
        if (data.error)
          return (
            <>
  ...
  return (
    <>
      {!Loading ? (
        <>
          <Head>
            <title>{getPageTitle('Sub Course')}</title>
          </Head>
          <SectionMain>
                  ...
                  <div className="grid gap-6 mb-6 md:grid-cols-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        id="phoneNumber"
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:bor"
                        placeholder="Any data"
                        required
                      />
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-center mt-6">
                        <input
                          ...
                          type="checkbox"
                          value=""
                          onClick={() => setIsWorkshop(!isWorkshop)}
...
SubCourse.getLayout = function getLayout(page: ReactElement) {
