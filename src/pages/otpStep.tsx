...
const VeryfyPage: any = () => {
  ...
  const handleLogin: any = (e: any) => {
    setLoading(true)
    e.preventDefault()
    ApiAddData('admin/otp', { otp, email }, (data, error) => {
      setLoading(false)
      setData(data)
      //console.log(error);
      ...
      <SectionFullScreen bg="purplePink">
        <CardBox className="w-11/12 shadow-2xl md:w-7/12 lg:w-6/12 xl:w-4/12 hover:bg-gray-100">
          <form>
            {error ? (
              <div role="alert">
                <div className="px-4 py-2 font-bold text-white bg-red-500 rounded-t">Wrong!!</div>
                <div className="px-4 py-3 text-red-700 bg-red-100 border border-t-0 border-red-400 rounded-b">
                  <p>You've enter wrong OTP check your email should have new One</p>
                </div>
              </div>
            ) : (
              ''
