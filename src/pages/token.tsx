...
import TableSampleClients from '../components/Table/token'
import { getPageTitle } from '../config'
import CardBoxModal from '../components/CardBox/Modal'
import { ApiGetCategory, ApiAddData } from '../api/api'
import Datepicker from 'tailwind-datepicker-react'
import { useRouter } from 'next/router'

const TokenPage = () => {
  const columns: Array<string> = ['Token', 'Expire at', 'subCourses', 'Created At', 'actions']
  const [subCourseData, setSubCourseData] = useState([])
  const router = useRouter()

  useEffect(() => {
    ApiGetCategory('subCourse', (data) => {
  ...
  const handleModalAction = async () => {
    setError(false)
    if (subCoursesId == 0 && expire_at == '') {
      setError(true)
      return
    }
    setLoading(true)
    await ApiAddData('token', { expire_at, subCoursesId }, (data) => {
      console.log(data)
      if (data.error)
        return (
          <>
  ...
  return (
    <>
      {!Loading ? (
        <>
          <Head>
            <title>{getPageTitle('Token Repo')}</title>
          </Head>
          <SectionMain>
            <SectionTitleLineWithButton icon={mdiTableBorder} title="Token Repo" main>
              <Button
            ...
            <CardBox className="mb-6" hasTable>
              <TableSampleClients columns={columns} subCourseData={subCourseData} />
              <CardBoxModal
                title="Add Token"
                buttonColor="info"
                buttonLabel="Done"
                  ...
                  <div className="grid gap-6 mb-6 md:grid-cols-3">
                    <div>
                      ...
                      <div>
                        <div className="relative max-w-sm">
                          <Datepicker
                            options={options}
                            onChange={(dateSelected: any) => setExpire_at(dateSelected)}
                            show={show}
                    ...
                    <div>
                      <label
                        ...
                        className="block mb-2 text-sm font-medium text-gray-900 dark:text-white"
                      >
                        Choose sub-course to apply this token on
                      </label>
                      <select
                        onClick={(e: any) => setSubCoursesId(parseInt(e.target.value))}
                        id="countries"
                        className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                      >
                        {subCourseData?.map((data: any) => (
