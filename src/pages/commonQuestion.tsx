...
import TableSampleClients from '../components/Table/CQ'
...
const TokenPage = () => {
  ...
  const handleModalAction = async () => {
    setError(false)
    if (questionEn == '' && questionAr == '' && answerAr == '' && answerEn == '') {
      setError(true)
      return
    }
    setLoading(true)
    await ApiAddData('commonQuestions', { questionEn, answerEn, questionAr, answerAr }, (data) => {
      console.log(data)
  ...
  return (
    <>
      {!Loading ? (
        <>
          <Head>
            <title>{getPageTitle('Common Questions')}</title>
          </Head>
          <SectionMain>
            <SectionTitleLineWithButton icon={mdiTableBorder} title="Common Questions" main>
              <Button
