...
import TableSampleClients from '../components/Table/payment'
...
const TablesPage = () => {
  const columns: Array<string> = ['title', 'description', 'detail1', 'detail2']
  ...
  const handleModalAction = async () => {
    setError(false)
    if (title == '' && description == '' && detail1 == '' && detail2 == '') {
      setError(true)
      return
    }
    setLoading(true)
    await ApiAddData('payments', { title, description, detail1, detail2 }, (data) => {
      if (data.error)
        return (
          <>
  ...
  return (
    <>
      {!Loading ? (
        <>
          <Head>
            <title>{getPageTitle('Payments Details')}</title>
          </Head>
          <SectionMain>
            <SectionTitleLineWithButton icon={mdiTableBorder} title="Payments Details" main>
              <Button
